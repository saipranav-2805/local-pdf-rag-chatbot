...
describe('Retrieval Graph Integration', () => {
    ...
    beforeAll(async () => {
        ...
        try {
            ...
            const testDocs = [
                new Document({
                    pageContent: 'Project XYZ-123 was initiated on March 15, 2024, with a budget of $2.5M. The project lead, Dr. Sarah Chen, established three primary objectives: improving system reliability by 35%, reducing operational costs by 28%, and implementing a new machine learning pipeline with 99.9% uptime.',
                    metadata: {
                        source: 'test',
                ...
                new Document({
                    pageContent: 'The quarterly security audit for Q1 2024 identified 17 low-priority vulnerabilities and 3 medium-priority issues. Team lead Michael Rodriguez implemented fixes for all medium-priority issues within 48 hours, achieving a new record response time. The remaining issues are scheduled for resolution by April 30, 2024.',
                    metadata: {
                        source: 'test',
    ...
    const shouldRunTests = () => {
        const hasEnvVars = process.env.SUPABASE_URL &&
    ...
    describe('Retrieval Path', () => {
        it('should retrieve and use context to answer specific project questions', async () => {
            ...
            const result = await graph.invoke({
                messages: [],
                query: 'What is the budget for Project XYZ-123 and who is the project lead?',
            });
            expect(result.messages).toHaveLength(1);
        ...
        it('should retrieve and provide security audit details', async () => {
            ...
            const result = await graph.invoke({
                messages: [],
                query: 'How many security vulnerabilities were found in Q1 2024 and who implemented the fixes?',
            });
            expect(result.messages).toHaveLength(1);
            expect(result.documents).toBeDefined();
            expect(result.documents.length).toBeGreaterThan(0);
        ...
        it('should handle questions without relevant context gracefully', async () => {
            ...
            expect(result.messages).toHaveLength(1);
            const content = String(result.messages[0].content);
            expect(content.toLowerCase()).toMatch(/(cannot|don't|not|sorry) (find|have|know|answer)/i);
        }, 30000);
    });
    ...
    describe('Message History', () => {
        it('should preserve human messages in the conversation history', async () => {
            ...
            const humanMessages = result2.messages.filter((msg) => msg instanceof HumanMessage);
            const aiMessages = result2.messages.filter((msg) => !(msg instanceof HumanMessage));
            ...
            result2.messages.forEach((msg, index) => {
                if (msg instanceof HumanMessage &&
                    ...
                    expect(result2.messages[index + 1] instanceof HumanMessage).toBe(false);
                }
            });
